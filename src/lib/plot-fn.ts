...
function isLinear(evalAt: PlotSeries["evalAt"]) {
  ...
  return Math.abs(y2 - 2 * y1 + y0) < 1e-6;
}
...
function buildWindow(
  ...
  for (const s of series) {
    ...
    const y0 = s.evalAt(0);
    if (y0 != null && Math.abs(y0) < 250 && (!s.clip || (0 >= s.clip.min - 1e-9 && 0 <= s.clip.max + 1e-9))) {
      xs.push(0);
      ys.push(y0);
...
export function extractPlots(html: string): PlotSpec | null {
  ...
  const reCount = new RegExp(FN_DEF.source, "g");
  let m: RegExpExecArray | null;
  while ((m = reCount.exec(html))) {
    const name = m[1]!;
    nameCount.set(name, (nameCount.get(name) || 0) + 1);
  }
  const re = new RegExp(FN_DEF.source, "g");
  while ((m = re.exec(html))) {
    const name = m[1]!;
    const variable = m[2]!;
    const rhs = m[3]!.trim();
